...
public class CartItemController {
    ...
    public void addCartItem(@PathVariable(value = "productId") int productId) {
        // Get the current customer from the spring security entity
        Authentication loggedInUser = SecurityContextHolder.getContext().getAuthentication();
        String username = loggedInUser.getName();
        Customer customer = customerService.getCustomerByUserName(username);

        // Get Customer cart and generate product
        Cart cart = customer.getCart();
        List<CartItem> cartItems = cart.getCartItem();
        Product product = productService.getProductById(productId);

        // Deal with repeatedly adding items
        ...
        // if not repeatedly adding items
